#!/usr/bin/env python
"""Launch the standalone single-case DICOM workflow."""
...
def _abort_frozen_import(message: str) -> None:
    ...
    if sys.platform == "win32":
        try:
            ...
            ctypes.windll.user32.MessageBoxW(0, message, "oppoCT-Single-Case-DICOM", 0x10)
        except Exception:
            pass
    raise SystemExit(1)
...
if getattr(sys, "frozen", False):
    try:
        ...
        _abort_frozen_import(
            ...
            "Rebuild the single-case package from the oppoCT repo using the "
            "same venv you use to run PyInstaller.\n\n"
            f"ImportError: {exc}"
        )

try:
    from scripts.single_case_workflow import launch_gui
