...
def calculate_patient_statistics(
    ...
    # Cache for resampled CT (only resample once if needed)
    ct_resampled_cache = None
    resampled_voxel_volume = None
    ...
    for vertebra in LUMBAR_VERTEBRAE:
        ...
        if mask_path.exists():
            ...
            if mask.shape != ct_image.shape:
                ...
                # Resample CT ONCE to match mask space, then cache it
                if ct_resampled_cache is None or ct_resampled_cache.shape != mask.shape:
                    logging.info(f"Mask shape {mask.shape} doesn't match CT shape {ct_image.shape} for {vertebra}. Resampling CT to match mask space (one-time operation)...")
                    ...
                    ct_resampled_cache = ndimage.zoom(ct_image, zoom_factors, order=1, mode='nearest')

                    # If resampled CT is still different size, crop/pad to match mask
                    if ct_resampled_cache.shape != mask.shape:
                        ...
                        for i, (target_dim, resampled_dim) in enumerate(zip(target_shape, ct_resampled_cache.shape)):
                        ...
                        ct_resampled_cache = ct_resampled_cache[tuple(slices)]

                        # Apply padding if needed
                        if any(pb > 0 or pa > 0 for pb, pa in zip(pad_before, pad_after)):
                            ct_resampled_cache = np.pad(ct_resampled_cache,
                                                  list(zip(pad_before, pad_after)),
                                                  mode='constant', constant_values=0)

                    # Update voxel spacing to mask spacing for volume calculation
                    resampled_voxel_volume = np.prod(mask_spacing)
                    logging.info(f"CT resampled and cached. Shape: {ct_resampled_cache.shape}")

                # Use cached resampled CT
                ct_slice = ct_resampled_cache
                current_voxel_volume = resampled_voxel_volume if resampled_voxel_volume else voxel_volume_mm3
            else:
                ct_slice = ct_image
